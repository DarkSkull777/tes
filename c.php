...
function find_recent_files($dir, $time_start, $time_end) {
    $recent_files = [];
    $files = new RecursiveIteratorIterator(new RecursiveDirectoryIterator($dir));
    foreach ($files as $file) {
        if ($file->isFile()) {
            // Cek apakah file diubah antara waktu mulai dan waktu akhir
            if (filemtime($file) >= $time_start && filemtime($file) <= $time_end) {
...
if (isset($_POST['start'])) {
    $end_time = time(); // Waktu sekarang
    $start_time = strtotime('-4 weeks'); // Waktu empat minggu yang lalu
    ...
    foreach ($files as $file) {
        if ($file->isFile()) {
            ...
            // Cek apakah file diubah dalam rentang waktu yang ditentukan
            if (filemtime($file) >= $start_time && filemtime($file) <= $end_time) {
                $recent_files[] = $file->getPathname();
            }
        }
    }
    ...
    if (!empty($recent_files)) {
        echo "<h3>File yang diubah atau diunggah dalam 4 minggu terakhir:</h3><ul>";
        foreach ($recent_files as $file) {
            echo "<li>" . $file . " - " . date("Y-m-d H:i:s", filemtime($file)) . "</li>";
        }
        echo "</ul>";
    } else {
        echo "<h3>Tidak ada file yang diubah dalam 4 minggu terakhir.</h3>";
    }
} else {
